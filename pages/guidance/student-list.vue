  ...
  <div class="h-auto w-full flex flex-col justify-center items-center mb-10">
    <div class="flex flex-row items-center justify-center w-5/6">
      <div class="w-1/3 flex flex-row justify-evenly">
        ...
        <GuidanceSortButton class="mr-0" @filter-selected="updateSortOption" />
      </div>
      <GuidanceSearchBar class="w-2/3" type="text" v-model="input" placeholder="Search Students..." />
    </div>
    <GuidanceStudentTable :viewall="viewAll" :new-students="sortedAndFilteredStudents.slice(x, y)" />
    <div class="max-w-[80%] overflow-x-auto mt-4 flex flex-row justify-between">
      <button v-if="currentChunk > 1" class="mx-2 bg-[#ebebeb] h-8 w-8 rounded-lg font-bold" @click="previousChunk">❮❮</button>
      <button class="mx-2 bg-[#ebebeb] h-8 w-8 rounded-lg font-bold" @click="previousPage" :disabled="currentPage === 1">❮</button>
      <button
      ...
      <button class="mx-2 bg-[#ebebeb] h-8 w-8 rounded-lg font-bold" :disabled="currentPage === totalPages" @click="nextPage">❯</button>
      <button v-if="currentChunk < totalChunks" class="mx-2 bg-[#ebebeb] h-8 w-8 rounded-lg font-bold" @click="nextChunk">❯❯</button>
    </div>
    <h5 class="mt-4">
      Page
...
useSeoMeta({
  title: "Student List | SITHS Course Selection"
});
...
async function fetchStudents() {
  ...
  try {
    ...
    loading.value = true;
    const profilesResponse = await fetch(`${import.meta.env.VITE_URL}/guidance/profiles`, {
      method: "GET",
      headers: {
...
function filterStudentsByCategory(students: studentGuidance[], sortBy: string) {
  ...
  switch (sortBy) {
    case "lastnameaz":
      categorizedStudents = categorizedStudents.sort((a: studentGuidance, b: studentGuidance) => a.name.localeCompare(b.name));
      break;
    case "lastnameza":
      categorizedStudents = categorizedStudents.sort((a: studentGuidance, b: studentGuidance) => b.name.localeCompare(a.name));
      break;
    case "ns":
      categorizedStudents = categorizedStudents.filter((student: studentGuidance) => student.status === "NOT STARTED");
      break;
    case "ip":
      categorizedStudents = categorizedStudents.filter((student: studentGuidance) => student.status === "INCOMPLETE");
      break;
    case "com":
      categorizedStudents = categorizedStudents.filter((student: studentGuidance) => student.status === "COMPLETE");
      break;
    case "final":
      categorizedStudents = categorizedStudents.filter((student: studentGuidance) => student.status === "FINALIZED");
      break;
    case "nine":
      categorizedStudents = categorizedStudents.filter((student: studentGuidance) => student.grade === "FRESHMAN");
      break;
    case "ten":
      categorizedStudents = categorizedStudents.filter((student: studentGuidance) => student.grade === "SOPHOMORE");
      break;
    case "eleven":
      categorizedStudents = categorizedStudents.filter((student: studentGuidance) => student.grade === "JUNIOR");
      break;
    case "transfer":
      categorizedStudents = categorizedStudents.filter((student: studentGuidance) => student.flag === "Transfer");
      break;
    case "regents":
      categorizedStudents = categorizedStudents.filter((student: studentGuidance) => student.flag === "Regents");
      break;
    case "sports":
      categorizedStudents = categorizedStudents.filter((student: studentGuidance) => student.flag === "Team");
      break;
    case "enl":
      categorizedStudents = categorizedStudents.filter((student: studentGuidance) => student.flag === "ENL");
      break;
  }
...
function filterStudentBySearch(students: studentGuidance[], query: string) {
  ...
  return students.filter((student: studentGuidance) => student.name.toLowerCase().includes(lowerCaseQuery) || student.email.toLowerCase().includes(lowerCaseQuery));
}
