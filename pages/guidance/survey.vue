    ...
    <div class="w-2/3">
      ...
      <div v-else>
        ...
        <div class="my-6">
          ...
          <SurveyDraggable :courses="surveyStore.currentResponse[indexAll].answer.preference" :index="indexAll" :numbered="true" :key="x" :color="'DEE9C8'"> </SurveyDraggable>
        </div>
        <div class="mt-14">
      ...
      <div class="flex justify-center mb-10 flex-col items-center">
        <p v-if="surveyStore.missingAnswers.length > 0" class="text-red-500 mb-4 text-center">Please answer all required questions before submitting.</p>
        <p v-else class="mb-4 text-center">After finalizing this survey, the student will no longer be able to edit their survey</p>
        <button @click="submit()" class="bg-[#DEE9C8] shadow-[2px_3px_2px_rgba(0,0,0,0.25)] w-36 h-12 text-2xl font-bold text-[#37394F]">Finalize</button>
      </div>
    </div>
  </section>
  <SurveyScrollPage :guidance="true" />
</template>
