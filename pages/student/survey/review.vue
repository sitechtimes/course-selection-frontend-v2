    ...
    <div class="lg:w-2/3 w-11/12">
      <div v-for="question in surveyStore.currentSurvey.question" :key="question.id" class="flex justify-center mb-8">
        <div class="flex flex-col w-11/12">
          <SurveyBoolean v-if="question.questionType === 'BOOLEAN'" :question="question" :warn="surveyStore.missingAnswers.filter((answer) => answer === question.id).length > 0 && shouldWarn" />
          <SurveyGeneral v-else-if="question.questionType === 'GENERAL'" :question="question" :warn="surveyStore.missingAnswers.filter((answer) => answer === question.id).length > 0 && shouldWarn" />
          <SurveyDropdown
      ...
      <div class="my-6">
        <p class="text-lg xl:leading-10 md:text-xl xl:text-3xl my-4">
          For the final part of the survey, please drag your classes in the order of priority, with the first choice being your top priority.
        </p>
        <SurveyDraggable :courses="ref_courses" :index="indexAllCourses" :numbered="true" :key="x" :color="'D6EEFF'" />
      </div>
      <div class="mt-14">
      ...
      <div class="flex justify-center my-10 flex-col items-center">
        <p v-if="surveyStore.missingAnswers.length === 0" class="mb-4 text-center">
          Once you submit, you will still be able to make changes to your survey before the due date. Once your guidance counselor finalizes your survey, you will not be able to edit.
        </p>
