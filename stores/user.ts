  ...
  actions: {
    ...
    async addFlag(email: string, newFlag: string, viewAll: boolean) {
      try {
        ...
        const studentIndex = this.currentlyViewingStudents.findIndex((student: studentPreview) => student.email + "@nycstudents.net" === email);
        const previewIndex = this.guidanceStudents.findIndex((student: studentGuidance) => student.email + "@nycstudents.net" === email);
