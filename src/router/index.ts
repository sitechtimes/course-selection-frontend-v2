					...
					children: [
						{
							...
							beforeEnter: async (to, from, next) => (useSurveyStore().open ? next() : next({ name: 'closedSurvey' })),
						},
						{
							path: 'closed',
							name: 'closedSurvey',
							component: () => import('../views/ClosedSurvey.vue'),
							beforeEnter: () => (useSurveyStore().open ? { name: 'studentSurvey' } : true),
						},
						{
							path: 'review',
							name: 'reviewSurvey',
...
router.beforeEach(async (to) => {
	...
	if (!anonPaths.includes(to.path) && !userStore.isAuth) return { name: 'login' };
	if (anonPaths.includes(to.path) && userStore.isAuth) return { name: `${userStore.isGuidance ? 'guidance' : 'student'}Dash` };
	if (to.meta.user === 'guidance' && !userStore.isGuidance) return { name: `error` };
	if (to.meta.user === 'student' && userStore.isGuidance) return { name: `error` };
});
