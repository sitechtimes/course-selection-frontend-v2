...
<template>
  <div class="h-auto select-none">
    ...
    >
      <div
        class="my-1.5 flex flex-row items-center justify-center"
        v-for="n in computedHeight"
        :key="n"
        :id="n.toString()"
      >
        <p>{{ n }}.</p>
        ...
        >
          <div
            ...
            @touchstart.prevent="(e) => handleTouchStart(e, n)"
            @touchmove.prevent="(e) => handleTouchMove(e)"
            @touchend.prevent="(e) => handleTouchEnd(e, n)"
          >
...
const computedHeight = computed(() => props.courses.length);
...
function updateRank(n: number, dragIndex: string | number) {
  ...
  if (
    props.index ===
    surveyStore.currentResponse.findIndex((x) => x.id === "allChosenCourses")
  ) {
