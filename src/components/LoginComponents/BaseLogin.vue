...
<template>
    <div id="studentLogin" class="w-full h-full flex flex-col justify-center relative items-center">
        ...
        <div class="flex flex-col w-11/12 max-w-[21rem] space-y-5 rounded-md">
            <LoginInput id="email" label="email" name="email" type="email" required :value="username" @update="newValue => username = newValue">Email</LoginInput>
            <LoginInput id="password" label="password" name="password" type="password" required :value="password" @update="newValue => password = newValue">Password</LoginInput>
        </div>
        <button class="bg-zinc-300 w-32 h-12 mt-4 rounded-md text-lg font-semibold hover:bg-zinc-200" @click="userStore.EmailLogin(username, password)">
            Login
        </button>
