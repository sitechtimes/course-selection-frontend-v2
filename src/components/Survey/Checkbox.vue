  ...
  <section class="lg:text-left text-center h-full w-full">
    <div class="flex flex-col lg:flex-row items-center lg:items-start w-full">
              ...
              <div v-for="choice in filteredCourses" :key="choice.name">
                <label
                  ...
                  <input type="checkbox" class="w-4 h-4 text-blue-400 border-gray-300 focus:ring-transparent"
                    :value="choice" v-model="courses"
                    :disabled="notInterested || (courses.length >= 3 && !courses.includes(choice))" />
                  {{ choice.name }}
                </label>
              </div>
              <label v-if="props.question.status === 'OPTIONAL'"
                class="text-base sm:text-lg xl:text-xl ml-4 flex flex-wrap flex-column justify-center items-center m-2 w-max gap-2">
                <input type="checkbox" class="w-4 h-4 text-blue-400 accent-black border-gray-300 focus:ring-transparent"
                  value="Not Interested" v-model="notInterested" />
                Not Interested
              </label>
            </div>
          </fieldset>
      ...
      <div
        ...
        <div class="">
          <p class="ml-10 mt-2 text-lg xl:leading-10 md:text-xl xl:text-2xl text-black max-w-[90%]">
            Drag course(s) into order of preference<span v-if="choices.length > 3" class="ml-1"> (top 3)</span>:
          </p>
          <SurveyDraggable class="p-6" :courses="draggableCourses" :answer="answer" :numbered="true" :color="color"
            :final="false" @save="emit('save')" />
        </div>
