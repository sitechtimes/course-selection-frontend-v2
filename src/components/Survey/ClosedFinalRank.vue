<template>
  <p class="text-lg xl:leading-10 md:text-xl xl:text-3xl my-4">
    Your final class priority:
  </p>
  <section class="lg:text-left text-center h-full w-full">
    <div class="flex flex-col lg:flex-row h-full w-full">
      <div
        class="flex items-center justify-center max-w-[40rem] overflow-hidden w-full"
      >
        <fieldset class="flex items-center justify-start w-full">
          <div
            class="text-left block py-2 px-3 mt-3 w-full md:w-3/5 text-base md:text-lg xl:text-2xl bg-transparent rounded-md border border-solid border-zinc-400 focus:outline-none focus:ring-0 focus:border-blue-400"
          >
            <div
              v-if="(surveyStore.answers[indexAll].answer as Rank[]).length != 0"
            >
              <p
                v-for="rank in (surveyStore.answers[indexAll].answer as Rank[])"
                :key="rank.rank"
                class="mb-2 mt-2 text-base md:text-lg xl:text-2xl"
              >
                <strong> {{ rank.rank }}. </strong>{{ findName(rank.course) }}
              </p>
            </div>
            <p v-else class="mt-2 mb-2 text-base md:text-lg xl:text-2xl">
              You have no classes chosen.
            </p>
          </div>
        </fieldset>
      </div>
    </div>
  </section>
</template>

<script setup lang="ts">
import { useSurveyStore } from "../../stores/survey";
import { Rank } from "../../types/interface";

const surveyStore = useSurveyStore();
const finalQuestion = surveyStore.survey.questions.find(
  (x) => x.questionType === "FINAL"
);
const indexAll = surveyStore.answers.findIndex(
  (x) => x.question === finalQuestion?.id
);
const findName = (id: number) =>
  surveyStore.coursesAvailable.find((x) => x.id === id)?.name;
</script>
