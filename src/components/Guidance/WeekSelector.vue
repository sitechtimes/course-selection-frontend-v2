<template>
    <div
      ...
      <div class="bg-white p-6 rounded-lg shadow-xl z-50 w-full max-w-xs">
        ...
        <p class="text-sm text-gray-600 mb-4 text-center">The calendar will jump to the week containing the selected date.</p>
  
        <div class="mb-4">
          <label for="week-select-date" class="block text-sm font-medium text-gray-700 mb-1">Date:</label>
          <input
            type="date"
            id="week-select-date"
  ...
  onMounted(() => {
    if (props.currentFirstDay) {
       ...
       const month = (props.currentFirstDay.getMonth() + 1).toString().padStart(2, '0');
