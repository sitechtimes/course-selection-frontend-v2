...
<template>
    <nav id="navbar" class="w-full top-0 h-[15vh] flex justify-between items-center px-8 md:px-12 lg:px-16 overflow-visible">
        ...
        <div v-if="viewingSurvey()" class="flex flex-row-reverse w-full sm:w-1/4 md:1/6 justify-between">
            ...
            <p v-if="(surveyStore.open && surveyStore.currentAnsweredSurvey.status != 'COMPLETE') || userStore.userType === 'guidance'" @click="surveyStore.saveSurvey('INCOMPLETE', surveyStore.currentAnsweredSurvey.grade); toggleSave()" class="text-[#37394F] text-2xl cursor-pointer hover:text-gray-500" ref="save">Save</p>
        </div>
