<template>
    <nav id="navbar"
        ...
        <div @click="redirect()" class="cursor-pointer">
            <p class="text-lg sm:text-xl md:text-2xl font-semibold z-50 hover:text-gray-600 flex sm:flex"
        ...
        <div v-if="userStore.isLoggedIn &&
            ...
            <p v-if="userStore.studentSurveyPreview !== null" @click="surveyNav()"
                class="cursor-pointer hover:text-gray-500">
                Survey
            </p>
            <RouterLink to="/">
                <p @click="logout()" id="name-link" class="text-base text-red-500 cursor-pointer hover:text-red-400">
                    Logout
                </p>
            </RouterLink>
        </div>
        <div v-if="userStore.isLoggedIn &&
            userStore.userType === 'guidance' &&
            viewingSurvey() === false
            " class="hidden justify-center items-center space-x-12 md:flex">
        ...
        <div v-if="viewingSurvey()" class="flex flex-row-reverse w-full sm:w-1/4 md:1/6 justify-between text-xl md:text-xl">
            <p @click="redirect()" class="text-[#37394F] cursor-pointer hover:text-gray-500">
                Exit
            </p>
            <p v-if="surveyStore.currentAnsweredSurvey.status === 'COMPLETE' &&
                surveyStore.open === true
                " @click="submit()" class="text-[#37394F] cursor-pointer hover:text-gray-500">
                Submit
            </p>
            <p v-if="surveyStore.currentAnsweredSurvey.status != 'COMPLETE' &&
                surveyStore.open === true
