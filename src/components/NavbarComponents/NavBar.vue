...
<template>
    <nav id="navbar"
        class="w-full top-0 h-[15vh] flex justify-between items-center px-8 md:px-12 lg:px-16 overflow-visible">
        ...
        <div v-if="userStore.isLoggedIn && userStore.userType === 'guidance' && viewingSurvey() === false"
            class="hidden justify-center items-center space-x-12 md:flex">
        ...
        <div v-if="!userStore.isLoggedIn && viewingSurvey() === false"
            class="hidden justify-center items-center space-x-12 md:flex">
            <p><a href="https://siths-catalog.netlify.app/" target="_blank" rel="noopener">Courses</a></p>
            <RouterLink to="/login">
    ...
    <div v-if="viewingSurvey()" class="flex flex-row-reverse w-full sm:w-1/4 md:1/6 justify-between">
        ...
        <p v-if="surveyStore.currentAnsweredSurvey.status === 'COMPLETE' && surveyStore.open === true" @click="submit()"
            class="text-[#37394F] text-2xl cursor-pointer hover:text-gray-500">Submit</p>
        <p v-if="surveyStore.currentAnsweredSurvey.status != 'COMPLETE' && surveyStore.open === true"
            @click="surveyStore.saveSurvey(surveyStore.currentAnsweredSurvey.status, surveyStore.currentAnsweredSurvey.grade); toggleSave()"
            class="text-[#37394F] text-2xl cursor-pointer hover:text-gray-500" ref="save">Save</p>
    </div>
