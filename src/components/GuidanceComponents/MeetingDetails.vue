  ...
  <body>
    <div v-if="show"
      ...
      <div class="event flex flex-col">
        ...
        <p class="ml-4 p-4">{{ meetingDetails.memo }}</p>
        <div>
        </div>
        <!-- buttons -->
        <div class="w-[60%] ml-4 mb-8 flex flex-row items-center">         
          ...
          <div
            class="item submit xl:text-2xl transition duration-300 hover:opacity-50 cursor-pointer w-fit">
            <button type="submit" @click="toggleDeleteModal"
              class="font-bold text-[1.2rem] bg-[#f28e8e] px-4 py-2 rounded-xl w-fit h-fit" id="save" ref="save">
              Delete
            </button>
          </div>
        </div>
