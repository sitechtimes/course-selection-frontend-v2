  ...
  <div class="h-[75vh] w-full flex flex-row justify-center items-center">
    ...
    <div v-else id="left" class="w-5/6 flex flex-col justify-center items-center text-center space-y-4 lg:items-start lg:text-left md:w-3/4 lg:max-w-2xl xl:max-w-3xl lg:space-y-6 lg:ml-12">
      ...
      <div id="announcements" class="text-lg md:text-xl text-left flex justify-center items-center ml-4 lg:ml-0 lg:justify-start">
        ...
        <h2 v-else-if="studentStore.studentSurveyPreview.status === 'FINALIZED'">Your guidance counselor has finalised your survey. If you wish to make changes, please contact them.</h2>
      ...
      <div v-else-if="studentStore.studentSurveyPreview.status === 'COMPLETE'" class="text-[#174616] bg-[#A8D480]  font-semibold text-center p-3 lg:px-6 lg:text-base text-sm rounded-md">Survey Status:
        <span class="font-medium">Submitted</span>
      </div>
      <div v-else-if="studentStore.studentSurveyPreview.status === 'INCOMPLETE'" class="text-[#461616] bg-[#F9D477] font-semibold text-center p-3 lg:px-6 lg:text-base text-sm rounded-md">Survey Status:
        <span class="font-medium">In Progress</span>
      </div>
      <div v-else-if="studentStore.studentSurveyPreview.status === 'FINALIZED'" class="text-[#461616] bg-[#D1A4DE] font-semibold text-center p-3 lg:px-6 lg:text-base text-sm rounded-md">Survey Status:
