<template>
  <div class="h-[75vh] w-full flex flex-row justify-center items-center">
    <div
      v-if="userStore.student.homeroom != null"
      id="left"
    ...
    >
      <h2 id="name" class="text-5xl font-bold">
        Welcome,
        {{
          userStore.first_name.toLowerCase().charAt(0).toUpperCase() +
          userStore.first_name.toLowerCase().slice(1)
        }}
        {{
          userStore.last_name.toLowerCase().charAt(0).toUpperCase() +
          userStore.last_name.toLowerCase().slice(1)
        }}.
      </h2>
      <div
        id="announcements"
    ...
    >
      ...
      >
        <BellIcon />
        <h2 v-if="surveyStore.open">
          Surveys are closing on {{ closeTime[1] }}/{{ closeTime[2] }}/{{
            closeTime[0]
          }}.
        </h2>
        <h2
          v-else-if="userStore.studentSurveyPreview[0].status === 'FINALIZED'"
        >
          Your guidance counselor has finalised your survey. If you wish to make
      ...
      <!-- survey status -->
      <div
        v-if="surveyStore.currentAnswers.answers.length === 0"
        class="text-[#461616] bg-[#EA9F9F] font-semibold text-center p-3 lg:px-6 lg:text-base text-sm rounded-md"
      >
        Survey Status:
        <span class="font-medium">Not Started</span>
      </div>
      <div
        v-else-if="userStore.studentSurveyPreview[0].status === 'COMPLETE'"
        class="text-[#174616] bg-[#A8D480] font-semibold text-center p-3 lg:px-6 lg:text-base text-sm rounded-md"
      >
        Survey Status:
        <span class="font-medium">Submitted</span>
      </div>
      <div
        v-else-if="userStore.studentSurveyPreview[0].status === 'INCOMPLETE'"
        class="text-[#461616] bg-[#F9D477] font-semibold text-center p-3 lg:px-6 lg:text-base text-sm rounded-md"
      >
        Survey Status:
      ...
      <div
        v-else-if="userStore.studentSurveyPreview[0].status === 'FINALIZED'"
        class="text-[#461616] bg-[#D1A4DE] font-semibold text-center p-3 lg:px-6 lg:text-base text-sm rounded-md"
      >
        Survey Status:
      ...
      <div>
        ...
        <p
          v-if="
            userStore.student.meeting != undefined ||
            userStore.student.meeting != null
          "
        >
...
if (userStore.student.homeroom === null) {
...
if (
  ...
  const datetime = new Date(userStore.student.meeting);
