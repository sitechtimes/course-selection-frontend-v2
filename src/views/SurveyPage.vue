...
onBeforeMount(() => {
  userStore.setSurvey(userStore.data.student.osis, userStore.data.survey.questions)
})
...
<template>
  <div class="h-screen flex flex-col justify-center items-center space-y-8">
    ...
    <div v-else class="w-11/12 md:w-4/5 lg:w-3/4 flex flex-col justify-center items-center min-h-[20rem] space-y-8 mb-10">
      <h1 class="text-4xl font-semibold">{{ userStore.data.survey.grade }} Year Survey</h1>
      <generalComponent v-if="currentQuestion.questionType === 'GENERAL'" :question="currentQuestion" :answers="currentAnswer" :key="currentQuestion.id"></generalComponent>
      <booleanComponent v-else-if="currentQuestion.questionType === 'BOOLEAN'" :question="currentQuestion" :answers="currentAnswer" :key="currentQuestion.question"></booleanComponent>
      <checkboxComponent v-else :question="currentQuestion" :choices="getChoices()" :answers="currentAnswer" :key="currentQuestion.questionType"></checkboxComponent>
    </div>
