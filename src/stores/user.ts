...
export const useUserStore = defineStore('user', () => {
	...
	async function login(username: string, password: string) {
		if (!username || !password) return setPopup('Username and password cannot be empty.', true);
		...
		firstName.value = data.firstName[0].toUpperCase() + data.firstName.slice(1).toLowerCase();
		lastName.value = data.lastName[0].toUpperCase() + data.lastName.slice(1).toLowerCase();
	...
	async function resetPasswordConfirm(new_password1: string, new_password2: string, token: string, uid: string) {
		if (new_password1 !== new_password2) return setPopup('Passwords do not match.', true);
		if (!new_password1 || !new_password2) return setPopup('Password cannot be empty.', true);
		if (!token || !uid) return setPopup('Invalid reset link. Try reseting again.', true);
		...
		if ('token' in data) return setPopup('Password already reset, please request another email.', true);
	...
	async function getSurveys() {
		const res = await fetchData('surveys/');
		if (!res.ok) return await res.json();
		const data = await res.json();
		return data;
	}
	...
	async function changeMeeting(id: number, deleteMeeting: boolean, date?: string, description?: string, notify?: boolean) {
		if (deleteMeeting) return;
		const res = await fetchData('guidance/updateMeeting/', deleteMeeting ? 'DELETE' : 'POST', { id, date, description, notify });
		if (!res.ok) return await res.json();
		const data = await res.json();
		console.log(data);
