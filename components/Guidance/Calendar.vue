        ...
        <div class="calendar w-full">
          ...
          <ul class="days">
            <li class="dayCon" v-for="h in calendarData.dateInfo" :key="h.id">
              ...
              <div v-for="meeting in h.meetings" :key="meeting.id" @click="toggleDetails(meeting.meetingDetails)">
                <p :class="`w-[100%] text-center truncate ${classColor[meeting.grade]} rounded-md p-1.5 mb-1 font-bold transition duration-500 hover:opacity-80 cursor-pointer hover:shadow-md`">
                  {{ meeting.name }}
                </p>
              </div>
              <IconsPlus class="plusIcon w-3 ml-2 cursor-pointer invisible" @click="toggleEvent(h)" />
            </li>
          </ul>
        </div>
        <GuidanceUpcomingMeetings />
      </div>
    </div>
    <GuidanceCreateEvent v-if="showEvent" :todaysDate="createEventDate" />
    <GuidanceMeetingDetails v-if="showDetails" :meetingDetails="meetingDetails" />
  </div>
</template>
...
const toggleEvent = (date: any) => {
  let year = todaysYear;
  let month = date.type == "previous" ? todaysMonth - 1 : date.type == "future" ? todaysMonth + 1 : todaysMonth;
  if (month == -1) {
    month = 11;
    year = todaysYear - 1;
  } else if (month == 12) {
...
const renderCalendar = async () => {
  ...
  for (let i = 1; i <= lastDateofMonth; i++) {
    ...
    for (const student of userStore.guidanceMeetings) {
      ...
      const isMeetingDate =
        studentMeetingDate.getDate() === activeDate.getDate() && studentMeetingDate.getMonth() === activeDate.getMonth() && studentMeetingDate.getFullYear() === activeDate.getFullYear();
      if (isMeetingDate) {
    ...
    studentsWithMeetings.sort((a, b) => {
      return new Date(a.meetingDetails.meetingTime).getTime() - new Date(b.meetingDetails.meetingTime).getTime();
    });
