      ...
      <form id="form" ref="form" @submit.prevent="empty()">
        <div class="times flex flex-col lg:flex-row">
          <div class="item mb-6">
            <label class="formt flex flex-row text-[#717494] ml-8 xl:text-2xl font-bold" for="date">
              ...
              Date</label
            >
            <input class="space d rounded-md border border-solid border-zinc-400 h-10 p-2 ml-6 mt-1 w-80" type="text" v-model="date" ref="dateElement" onfocus="(this.type='date')" id="date" />
        ...
        <div class="item mb-6">
          ...
          <datalist id="suggestions">
            <option v-for="student in studentList" :key="student.email">{{ titleCaseName(student.name) }}, | {{ student.email }}@nycstudents.net</option>
          </datalist>
        ...
        <div class="item mb-6">
          ...
          <input class="space rounded-md border border-solid border-zinc-4a00 w-80 h-10 p-2 ml-6 mt-1" type="text" v-model="description" placeholder="Memo" />
        </div>
        ...
        <div class="item submit ml-6 mb-6 xl:text-2xl transition duration-300 hover:opacity-50 cursor-pointer w-fit">
          <button type="submit" class="text-[1.5rem] mb-8 duration-300 font-bold bg-primary-g px-4 py-2 rounded-xl w-fit h-fit opacity-0.5:hover" id="save" ref="save">Save</button>
        </div>
      </form>
    </div>
  </div>
