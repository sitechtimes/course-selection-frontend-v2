  ...
  <div class="flex flex-col mt-6 justify-center items-center w-5/6" id="table">
    <table class="w-full border-2 border-black table-auto text-left">
      <thead class="fill-primary-g border-2 border-black">
        <tr class="bg-primary-g">
          ...
          <th class="p-4 flex flex-row items-center">
            ...
            <div>
              <img src="../icons/InfoCircle.png" alt="hover for more information on student flags" class="h-5" @mouseover="tooltip = true" @mouseout="tooltip = false" />
      ...
      <tbody v-for="student in newStudents" :key="student.email" class="border-2 border-black">
        <GuidanceAddFlag v-if="showFlagModal === student.email" @exit="toggleFlagModal('')" :student="student" :viewAll="viewall" :flags="flags"> </GuidanceAddFlag>
        <GuidanceDeleteFlag v-if="showDeleteFlag === student.email" @exit="toggleDeleteFlag('')" :student="student" :viewAll="viewall" :flags="flags"> </GuidanceDeleteFlag>
        <tr>
          <td class="p-4">
          ...
          <td class="p-4 flex flex-row parent items-center">
            <div v-for="flag in flags" :key="flag.flag">
              <div>
                <div id="flagbox" v-show="student.flag.includes(flag.flag)" :title="flag.title" :class="`${flag.color}`" class="m-1 rounded-full h-5 w-5"></div>
              </div>
            </div>
            <IconsPlus @click="toggleFlagModal(student.email)" class="w-3 m-1 hidden child hover:cursor-pointer"> </IconsPlus>
            <IconsMinus @click="toggleDeleteFlag(student.email)" class="w-3 m-1 hidden child hover:cursor-pointer"> </IconsMinus>
          </td>
        </tr>
      </tbody>
    </table>
