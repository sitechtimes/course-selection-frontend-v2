<template>
  <nav id="navbar" class="w-full top-0 h-[13vh] md:h-[15vh] flex justify-between items-center px-8 md:px-12 lg:px-16 overflow-visible">
    <div @click="redirect()" class="cursor-pointer">
      <p class="text-lg sm:text-xl md:text-2xl font-semibold z-50 hover:text-gray-600 flex sm:flex" :class="{ hidden: viewingSurvey() }">Course Selection</p>
    </div>
    <div v-if="userStore.isLoggedIn && userStore.userType === 'student' && viewingSurvey() === false" class="hidden justify-center items-center space-x-12 md:flex">
      ...
      <p v-if="userStore.studentSurveyPreview !== null" @click="surveyNav()" class="cursor-pointer hover:text-gray-500">Survey</p>
      <NuxtLink to="/">
    ...
    <div v-if="userStore.isLoggedIn && userStore.userType === 'guidance' && viewingSurvey() === false" class="hidden justify-center items-center space-x-12 md:flex">
      ...
      <NuxtLink to="/guidance/calendar">
        <p class="text-base hover:text-gray-500">Calendar</p>
      </NuxtLink>
      <NuxtLink id="link" to="/guidance/statistics">
    ...
    <div v-if="!viewingSurvey()" id="menu-icon" class="flex justify-center items-center cursor-pointer z-40 md:hidden">
      <IconsMenuIcon @click="toggleMenu" v-if="!menuOpen" />
      <IconsCloseMenu @click="toggleMenu" v-else />
    </div>
    ...
    <NavBarMobile v-if="menuOpen" @e="toggleMenu" />
  </nav>
</template>
